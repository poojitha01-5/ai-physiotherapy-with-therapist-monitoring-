...
export default function WarriorPose() {
  ...
  const [backLegAngle, setBackLegAngle] = useState<number>(180);
  const [armAngle, setArmAngle] = useState<number>(180);
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === "frame") {
          ...
          setBackLegAngle(data.back_leg_angle || 180);
          setArmAngle(data.arm_angle || 180);
        ...
        <div className="flex flex-col items-center justify-center h-[80vh] bg-gray-900 rounded-lg overflow-hidden relative">
          ...
          <div className="absolute top-4 right-4 bg-black/70 backdrop-blur-md p-4 rounded-lg text-white max-w-xs z-10">
            ...
            <ul className="text-sm space-y-1">
              <li>1. Stand with wide stance</li>
              <li>2. Turn one foot out 90°</li>
              <li>3. Bend front knee to 90°</li>
              <li>4. Keep back leg straight</li>
              <li>5. Extend arms parallel to ground</li>
              <li>6. Face forward, gaze steady</li>
            </ul>
          </div>
          ...
          {calibrationStatus === "passed" && isRunning && (
            <div className="absolute bottom-4 left-4 bg-black/70 backdrop-blur-md p-4 rounded-lg text-white z-10">
              <h3 className="text-lg font-bold mb-2 text-blue-400">Pose Data</h3>
              <p className="text-sm">Front Knee: <span className="font-semibold">{frontKneeAngle.toFixed(1)}°</span></p>
              <p className="text-sm">Back Leg: <span className="font-semibold">{backLegAngle.toFixed(1)}°</span></p>
              <p className="text-sm">Arm Angle: <span className="font-semibold">{armAngle.toFixed(1)}°</span></p>
            </div>
          )}
