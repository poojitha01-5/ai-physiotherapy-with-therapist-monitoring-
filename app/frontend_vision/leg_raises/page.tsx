...
export default function LegRaises() {
  ...
  const [legAngle, setLegAngle] = useState<number>(180);
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === "frame") {
          ...
          setLegAngle(data.leg_angle || 180);
        ...
        <div className="flex flex-col items-center justify-center h-[80vh] bg-gray-900 rounded-lg overflow-hidden relative">
          ...
          <div className="absolute top-4 right-4 bg-black/70 backdrop-blur-md p-4 rounded-lg text-white max-w-xs z-10">
            ...
            <ul className="text-sm space-y-1">
              <li>1. Lie on your back</li>
              <li>2. Keep legs straight</li>
              <li>3. Raise one leg slowly</li>
              <li>4. Raise to 30-45°</li>
              <li>5. Hold briefly</li>
              <li>6. Lower leg slowly</li>
            </ul>
          </div>
          ...
          {calibrationStatus === "passed" && isRunning && (
            <div className="absolute bottom-4 left-4 bg-black/70 backdrop-blur-md p-4 rounded-lg text-white z-10">
              ...
              <p className="text-sm">Reps: <span className="font-semibold">{repCount}/{targetReps}</span></p>
              <p className="text-sm">Leg Angle: <span className="font-semibold">{legAngle.toFixed(1)}°</span></p>
